...
Outra forma de utilizar middlewares é quando precisamos reaproveitar um middleware para todas as rotas da nossa aplicação, ou uma boa parte destas. Vamos criar uma forma de autenticar se um determinado usuário pode ter acesso a nossa API de recipes. Vamos enviar as informações de nome de usuário e senha pelo Header da requisição (⚠️ Este é um exemplo didático, na prática vamos utilizar abordagens mais seguras de fazer esse tipo de autenticação, por exemplo utilizando JWT).
Vamos começar definindo nosso middleware. */
...
/* No código acima temos um middleware que, ao receber uma request, verifica se essa request contém as informações username e password no header da requisição. Se alguma das informações não foi enviada, retorna uma mensagem dizendo que essas informações não podem vir vazias, e caso seja enviado ainda é feita uma verificação para saber se correspondem a um usuário específico.
...
Para utilizarmos esse middleware de autenticação, podemos fazer da seguinte forma: */
